...
*/
  ...
  if (data[0]=='Z') {   // If the message begins with a 'Z', it's an identification message 
    ...
    LOG_INFO("\nSending response.\nb");
    snprintf(buffer_message, sizeof(buffer_message), "OK");
    simple_udp_sendto(&udp_conn, buffer_message, strlen(buffer_message), sender_addr);
  } 
...
PROCESS_THREAD(udp_server_process, ev, data) {
  ...
  while (1) {
    ...
    if (ev==serial_line_event_message && data != NULL) {
      ...
      index=6;
      while (index<20 && rasp_msg[index]!=',') {    // Reading the JSON message
      ...
      if (check_ID==1) {
        printf("\nEnvoi de %s à l'adresse : ",(char *) data);
